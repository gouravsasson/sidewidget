...
const German = () => {
  ...
  useEffect(() => {
    if (status === "disconnected") {
      setSpeech(
        ` ${widgetTheme?.bot_name || "Mit dem KI-Assistenten sprechen"}`
      );
    } else if (status === "connecting") {
      setSpeech(
        `Verbindung wird hergestellt mit ${
          widgetTheme?.bot_name || "KI-Assistent"
        }`
      );
    } else if (status === "speaking") {
      setSpeech(`${widgetTheme?.bot_name || "KI-Assistent"} spricht`);
      setExpanded(true);
    } else if (status === "connected") {
      setSpeech(`Verbunden mit ${widgetTheme?.bot_name || "KI-Assistent"}`);
    } else if (status === "disconnecting") {
      setSpeech(`Verbunden mit ${widgetTheme?.bot_name || "KI-Assistent"}`);
    } else if (status === "listening") {
      setSpeech(`${widgetTheme?.bot_name || "KI-Assistent"} hört zu`);
    }
                  ...
                  >
                    {formSubmitting ? (
                      <div className="flex items-center justify-center">
                        ...
                        Verbindung mit KI-Assistent wird hergestellt
                      </div>
                    ) : (
                      "Absenden"
                    )}
              ...
              <>
                ...
                {widgetTheme?.bot_show_transcript && (
                  <div className="px-6 py-4 flex-1">
                    ...
                    >
                      {transcripts || "Ihr Gespräch erscheint hier..."}
                    </div>
                  </div>
                )}
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          >
            {widgetTheme?.bot_name ||
              ` ${widgetTheme?.bot_name || "Mit KI-Assistent sprechen"}`}
          </div>
        </div>
      )}
    </div>
